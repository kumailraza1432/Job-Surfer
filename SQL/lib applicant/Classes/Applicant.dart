...
class Applicant extends User {
  ...
  getApplications(context)async{
    ...
    var r = await MySQL.query("SELECT username,Status,Title,EmployerNote FROM ( Select JobAdID,EmployerNote,Status from sql6459853.Application where CNIC=$CNIC) as a natural join sql6459853.JobAd natural join sql6459853.User ;", context,removePreContext: false);
    if(r is bool ) {
      return list;
    }
    for(var row in r) list.add(ApplicationWidget(row[0], row[2], row[1], row[3])) ;
    return list;
  }
